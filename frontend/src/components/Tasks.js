...
import '../components/Styles/Tasks.css'; // Ensure correct path to your Tasks.css
        ...
        <div>
          {tasks.length === 0 ? (
            ...
            tasks.map((task, index) => (
              <div key={task._id} className="task-item bg-white my-4 p-4 text-gray-600 rounded-md shadow-md">
                <div className="flex justify-between items-center">
                  <span className="font-medium">Task #{index + 1}</span>
                  <span className="text-gray-500 text-sm">{format(new Date(task.createdAt), 'PPP')}</span>
                </div>
